...
/*Specifiers*/
Specifier: TYPE {std::string temp=std::string($1);$$=new NSpecifier(temp);$$->line=yylineno;}   
        | StructSpecifier {$$=$1;$$->line=$1->line;}
        ;

StructSpecifier: STRUCT OptTag LC DefList RC {$$=new NStructSpecifier($2,$4);if($2)$$->line=$2->line;}   
        | STRUCT Tag {$$=new NStructSpecifier($2);$$->line=$2->line;}   
        ;

OptTag: ID {$$=new NIdentifier(1,std::string($1));$$->line=yylineno-1;}   
        | {$$=nullptr;}
        ;

Tag: ID {$$=new NIdentifier(2,std::string($1));$$->line=yylineno;}   
        ;

/*Declarators*/
VarDec:  ID {auto t=new NIdentifier(4,std::string($1));$$=new NVarDec(*t);$$->line=yylineno;}   //ID结点，标识符符号串存放结点的type_id
        | VarDec LB INT RB {$1->v.push_back($3);}
        ;
FunDec: ID LP VarList RP {auto t=new NIdentifier(3,std::string($1));$$=new NFunDec(*t,$3);$$->line=yylineno;}
        | ID LP RP {auto t=new NIdentifier(3,std::string($1));$$=new NFunDec(*t,nullptr);$$->line=yylineno;}
        ;
...
/*Statements*/
CompSt:   LC DefList StmtList RC {$$=new NCompSt($2,$3);if($2)$$->line=$2->line-1;else if($3)$$->line=$3->line-1;}
        ;
StmtList: {$$=nullptr;}  
...
Exp:      Exp ASSIGNOP Exp {$$=new NAssignment(*(new std::string("ASSIGNOP")),*$1,ASSIGNOP,*$3);$$->line=yylineno;}//$$结点type_id空置未用，正好存放运算符
        ...
        | ID LP Args RP {auto t=NIdentifier(0,std::string($1));$$=new NMethodCall(t,$3);$$->line=yylineno;}
        | ID LP RP      {auto t=NIdentifier(0,std::string($1));$$=new NMethodCall(t);$$->line=yylineno;}
        | Exp LB Exp RB {$$=new NListOperator(*$1,*$3);$$->line=yylineno;}
        | Exp DOT ID    {auto t=NIdentifier(0,std::string($3));$$=new NDotOperator(*$1,t);$$->line=yylineno;}
        | ID            {$$=new NIdentifier(0,std::string($1));$$->line=yylineno;}
