...
OptTag: ID {$$=new NIdentifier(std::string($1));$$->line=yylineno;}   
        | {$$=nullptr;}
        ;

Tag: ID {$$=new NIdentifier(std::string($1));$$->line=yylineno;}   
        ;

/*Declarators*/
VarDec:  ID {auto t=new NIdentifier(std::string($1));$$=new NVarDec(*t);$$->line=yylineno;}   //ID结点，标识符符号串存放结点的type_id
        | VarDec LB INT RB {$1->v.push_back($3);}
        ;
FunDec: ID LP VarList RP {auto t=new NIdentifier(std::string($1));$$=new NFunDec(*t,$3);$$->line=yylineno;}
        | ID LP RP {auto t=new NIdentifier(std::string($1));$$=new NFunDec(*t,nullptr);$$->line=yylineno;}
        ;
...
/*Statements*/
CompSt:   LC DefList StmtList RC {$$=new NCompSt($2,$3);$$->line=$2->line-1;}
        ;
StmtList: {$$=nullptr;}  
...
Exp:      Exp ASSIGNOP Exp {$$=new NAssignment(*(new std::string("ASSIGNOP")),*$1,ASSIGNOP,*$3);$$->line=yylineno;}//$$结点type_id空置未用，正好存放运算符
        ...
        | Exp RELOP Exp {$$=new NBinaryOperator(*(new std::string($2)),*$1,*$2,*$3);$$->line=yylineno;}  //词法分析关系运算符号自身值保存在$2中
        ...
        | ID LP Args RP {auto t=NIdentifier(std::string($1));$$=new NMethodCall(t,$3);$$->line=yylineno;}
        | ID LP RP      {auto t=NIdentifier(std::string($1));$$=new NMethodCall(t);$$->line=yylineno;}
        | Exp LB Exp RB {$$=new NListOperator(*$1,*$3);$$->line=yylineno;}
        | Exp DOT ID    {auto t=NIdentifier(std::string($3));$$=new NDotOperator(*$1,t);$$->line=yylineno;}
        | ID            {$$=new NIdentifier(std::string($1));$$->line=yylineno;}
